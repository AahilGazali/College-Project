...
class PDFExtractor:
    def __init__(self, mongodb_uri: str, database_name: str, google_drive_key: Optional[str] = None):
        """Initialize the PDF Extractor with MongoDB connection."""
        self.client = pymongo.MongoClient(mongodb_uri)
        self.db = self.client[database_name]
        self.documents = self.db.documents

        # Optionally provided google drive credentials path
        self.google_drive_key = google_drive_key
        ...
        # Only attempt to load credentials if we have a key path
        try:
            self.load_credentials()
        except Exception as e:
            logger.warning('Google Drive credentials not loaded: %s', str(e))

    def load_credentials(self):
        """Load or create Google Drive credentials."""
        creds = None
        # Prefer credentials path passed via constructor; fall back to repo location
        creds_path = self.google_drive_key or os.path.join(os.path.dirname(__file__), '..', 'google-drive-key.json')
    ...
    def download_from_drive(self, file_id: str) -> str:
        ...
        try:
            ...
            local_dir = os.path.join(os.getcwd(), 'server', 'uploads', 'documents')
...
if __name__ == "__main__":
    # Load configuration (from same folder as this script)
    ...
    extractor = PDFExtractor(
        ...
        google_drive_key=(config.get('google_drive') or {}).get('credentials_path')
    )

    # Simple CLI so this script can be used as a worker from Node
    import argparse

    parser = argparse.ArgumentParser(description='PDF Processor CLI')
    parser.add_argument('--file-id', help='Google Drive file id to download and process')
    parser.add_argument('--local-path', help='Local PDF path to process')
    args = parser.parse_args()

    try:
        if args.local_path:
            extracted = extractor.extract_text_from_pdf(args.local_path)
            print(json.dumps({
                'success': True,
            ...
            # download to local path then extract
            local_path = extractor.download_from_drive(args.file_id)
            extracted = extractor.extract_text_from_pdf(local_path)
            # cleanup
            try:
                if os.path.exists(local_path):
                    os.remove(local_path)
            ...
            print(json.dumps({'success': False, 'error': 'No --local-path or --file-id provided'}))
    except Exception as e:
